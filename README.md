...
| ワンセグが実際に映る | **未解決。ここが本丸** | 復調・TS 処理・DRM はすべて純正のクローズドな部分にあり、過去の d2dcm 移植でも誰も移植していない |
...
**先に悪い話をします。** ワンセグを新しい Android で動かすのは、
LineageOS を入れること自体よりはるかに難しく、**成功が保証できません**。

理由は 3 つで、いずれも「頑張れば終わる作業」ではなく「壁」です。
...
3. **RMP（コンテンツ権利保護専用方式）がある。**
   2012年8月から順次導入された地上デジタルの DRM で、B-CAS カードの代わりに
   **受信機に埋め込まれた鍵データ**で復号します。この鍵ごと移植できないと、
   受信できても映像が復号できません。

その上で、**やる価値はあります**。詳細な攻略手順を
[`docs/05-ワンセグ移植の手順.md`](docs/05-ワンセグ移植の手順.md) に書きました。
最初の一歩は「純正 ROM のワンセグスタックが何でできているかを実機から吸い出す」ことで、
そのためのスクリプトを [`tools/oneseg-probe.sh`](tools/oneseg-probe.sh) に用意しました。

**ワンセグが最優先なら**、純正 4.1.2 のまま root だけ取る、という選択肢も真剣に検討してください。
...
kernel/patches/                  カーネルパッチ（現状ドライバ有効化は不要。理由は README 参照）
manifests/                       repo sync 用の local_manifest
tools/
  oneseg-probe.sh                純正 ROM のワンセグスタックを実機から調査する
  extract-oneseg-blobs.sh        調査結果をもとに blob を抜き出す
  verify-tree.sh                 デバイスツリーの静的チェック
```
...
**純正 4.1.2 のまま、root を取った SC-06D** で実行してください。
